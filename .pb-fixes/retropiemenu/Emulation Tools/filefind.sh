...
# Dual Script with USB Roms check, 07.05.26 By 2Play! (Based/Using Original Creator's script)
...
infobox="${infobox}\n"
infobox="${infobox}Dual Option Script to check/fix also USB Roms setup by 2Play!\n"
infobox="${infobox}Based On Cyperghost's script)\n"
infobox="${infobox}\n"
...
--title "CUSTOM COLLECTIONS ROMS PATH FIX" \
...
function main_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "$BACKTITLE" --title " CUSTOM COLLECTIONS ROMS PATH FIX MENU " \
            --ok-label OK --cancel-label Exit \
            --menu "Are you ready?" 25 75 20 \
            - "*** CUSTOM COLLECTIONS PATH FIX ***" \
            ""      "" \
			1 " - Fix Imported Collections Paths - RetroPie Roms" \
            2 " - Fix Imported Collections Paths - External USB Roms" \
            3 " - Fix Imported Collections Paths - localroms of USB" \
            2>&1 > /dev/tty)

        case "$choice" in
		1) fix_collections "/home/pi/RetroPie/roms" ;;
		2) fix_collections "/home/pi/addonusb/roms" ;;
		3) fix_collections "/home/pi/RetroPie/localroms" ;;
		-) none ;;
		*) break ;;
		esac
    done
...
function fix_collections() {
	local rombase="$1"   # pass in ROMBASE_DIR
    readonly VERSION="1.10_060918"
    readonly TITLE="Pathfinder 4 Custom Collections"
    readonly ROMBASE_DIR="$rombase"
